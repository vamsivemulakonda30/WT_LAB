...
app.get('/logout', function(req, res) {
 ...
 {
    respond.sendFile(__dirname+"/i2.png");
 });
 app.get('/facultypng',(request,respond)=>{
    respond.sendFile(__dirname+"/i1.png");
 });
...
app.post('/update',(req, res) => {
  ...
  const sql = `
    UPDATE timetable WHERE Year = ? AND Section = ? AND Day = ? AND Period = ? `;

  connection.query(sql, [Year,Section,Day,Period], (err, result) => {
    if (err) throw err;
